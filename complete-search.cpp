...
void visit(int n, int i, int j, int* visitCounter, int** grid, int nRows, int nCols, bool print=false) {
    
    grid[i][j] += n;
    *visitCounter += n;
    
    if (print) printState(grid, nRows, nCols);
...
void pathSearch(std::pair<int, int> dir, int i, int j, int nRows, int nCols, int** grid, int* visitCounter, int* solveCounter) {
    ...
    if (!reachedBounds(i, j, dir, nRows, nCols) &&
        ...
        visit(1, i + dir.first, j + dir.second, visitCounter, grid, nRows, nCols);
    ...
    if (!reachedBounds(i, j, swapped_dir, nRows, nCols) &&
        ...
        visit(1, i + swapped_dir.first, j + swapped_dir.second, visitCounter, grid, nRows, nCols);
        pathSearch(swapped_dir, i + swapped_dir.first, j + swapped_dir.second, nRows, nCols, grid, visitCounter, solveCounter);
        visit(-1, i + swapped_dir.first, j + swapped_dir.second, visitCounter, grid, nRows, nCols);
    ...
    if (!reachedBounds(i, j, invertedSwapped_dir, nRows, nCols) &&
        ...
        visit(1, i + invertedSwapped_dir.first, j + invertedSwapped_dir.second, visitCounter, grid, nRows, nCols);
        pathSearch(invertedSwapped_dir, i + invertedSwapped_dir.first, j + invertedSwapped_dir.second, nRows, nCols, grid, visitCounter, solveCounter);
        visit(-1, i + invertedSwapped_dir.first, j + invertedSwapped_dir.second, visitCounter, grid, nRows, nCols);
    }
}
